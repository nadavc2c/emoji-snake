# Emoji art attribution

The emoji tile images in this folder are from **OpenMoji** - the open-source emoji and
icon project.

- Source: https://openmoji.org (`color/618x618` PNGs, by Unicode code point)
- License: **CC BY-SA 4.0** - https://creativecommons.org/licenses/by-sa/4.0/

Every `*.png` in this folder is an unmodified OpenMoji color image named by its Unicode code
point. Roles in the game:

| File        | Emoji | Role                          |
|-------------|-------|-------------------------------|
| `1F40D.png` | 🐍    | Snake head                    |
| `1F7E2.png` | 🟢    | Snake body segment            |
| `1F331.png` | 🌱    | Background texture            |
| `1F34E.png` | 🍎    | Food (apple)                  |
| `1F355.png` | 🍕    | Food (pizza)                  |
| `1F354.png` | 🍔    | Food (burger)                 |
| `1F32E.png` | 🌮    | Food (taco)                   |
| `1F369.png` | 🍩    | Food (donut)                  |
| `1F356.png` | 🍖    | Food (meat)                   |
| `1F911.png` | 🤑    | Time-limited bonus            |
| `1F5FF.png` | 🗿    | Shed gag: the head/neck you bite to reattach |
| `1F4D6.png` | 📖    | Rare book (opens visual novel)|
| `1F414.png` | 🐔    | Basilisk head (chicken gag)   |
| `1F95A.png` | 🥚    | Egg (reverts the basilisk)    |
| `1F3EA.png` | 🏪    | On-board store (buy power-ups) |
| `1F3B0.png` | 🎰    | Gamble food (random length added) |
| `1FAB1.png` | 🪱    | Living food (flees)           |
| `1F42D.png` | 🐭    | Living food (flees)           |
| `1F424.png` | 🐤    | Living food (flees)           |
| `2764.png`  | ❤️    | Store: buy an extra life      |
| `2702.png`  | ✂️    | Store: THE BARBER (persistent haircut) |
| `1F9F1.png` | 🧱    | Obstacle (brick)              |
| `1FAA6.png` | 🪦    | Obstacle (headstone)          |
| `1F480.png` | 💀    | Obstacle (skull)              |
| `1F525.png` | 🔥    | Power-up - burn               |
| `1F9CA.png` | 🧊    | Power-up - slow               |
| `1F47B.png` | 👻    | Power-up - ghost              |
| `1FA99.png` | 🪙    | Power-up - gold (score x2)    |
| `1F9F2.png` | 🧲    | Power-up - magnet             |
| `1F300.png` | 🌀    | Portal                        |
| `1F479.png` | 👹    | Boss                          |
| `1F534.png` | 🔴    | Boss projectile               |
| `1F4A5.png` | 💥    | Impact                        |
| `1F48E.png` | 💎    | (legacy gem, retained)        |
